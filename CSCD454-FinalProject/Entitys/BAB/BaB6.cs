...
namespace CSCD454_FinalProject.Entitys
{
    public class BaB6 : BaseAttackBonus
    {
        ...
        public int[] getBaB(int level)
        {
            ...
            if (Math.Floor(level * 0.5) < 6)
            {
                BaB = new int[] { (int)Math.Floor(level * 0.5) };
                return BaB;
            }
            else if (Math.Floor(level * 0.5) < 11)
            {
                BaB = new int[] { (int)Math.Floor(level * 0.5), (int)Math.Floor((level - 5) * 0.5) };
                return BaB;
            }
            else if (Math.Floor(level * 0.5) < 16)
            {
                BaB = new int[] { (int)Math.Floor(level * 0.5), (int)Math.Floor((level - 5) * 0.5), (int)Math.Floor((level - 10) * 0.5) };
                return BaB;
            }
            else
            {
                BaB = new int[] { (int)Math.Floor(level * 0.5), (int)Math.Floor((level - 5) * 0.5), (int)Math.Floor((level - 10) * 0.5), (int)Math.Floor((level - 15) * 0.5) };
                return BaB;
            }
        }
    }
